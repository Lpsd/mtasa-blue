 ...
 *  FILE:        gui/CGUIGridListItem_Impl.cpp
...
CGUIGridListItem_Impl::CGUIGridListItem_Impl(CGUI_Impl* pGUI, const char* szText, unsigned int uiType, CGUIStaticImage_Impl* pImage)
{
    ItemType = uiType;
    ...
    {
        case TextItem:
            m_pListItem = (CEGUI::ListboxItem*)new CEGUI::StandardItem(CGUI_Impl::GetUTFString(szText));
            break;
        case ImageItem:
            m_pListItem = (CEGUI::ListboxItem*)new CEGUI::StandardItem(CGUI_Impl::GetUTFString(szText));
...
CGUIGridListItem_Impl::~CGUIGridListItem_Impl()
{
...
void CGUIGridListItem_Impl::SetDisabled(bool bDisabled)
{
    reinterpret_cast<CEGUI::ListboxItem*>(m_pListItem)->setDisabled(bDisabled);
}

void CGUIGridListItem_Impl::SetFont(const char* szFontName)
{
    if (szFontName)
...
void CGUIGridListItem_Impl::SetData(const char* pszData)
{
    if (pszData)
    {
        m_strData = pszData;
...
void CGUIGridListItem_Impl::SetImage(CGUIStaticImage* pImage)
{
...
std::string CGUIGridListItem_Impl::GetText() const
{
    return (const char*)CGUI_Impl::GetUTFString((const char*)m_pListItem->getText().c_str()).c_str();
}

CEGUI::ListboxItem* CGUIGridListItem_Impl::GetListItem()
{
    return m_pListItem;
}

void CGUIGridListItem_Impl::SetColor(unsigned char ucRed, unsigned char ucGreen, unsigned char ucBlue, unsigned char ucAlpha)
{
...
bool CGUIGridListItem_Impl::GetColor(unsigned char& ucRed, unsigned char& ucGreen, unsigned char& ucBlue, unsigned char& ucAlpha)
{
    if (ItemType == TextItem || ItemType == NumberItem)
    {
