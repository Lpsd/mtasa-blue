...
void CGUIGridList_Impl::Clear()
{
    try
    {
        ...
        CFastHashMap<CEGUI::ListboxItem*, CGUIGridListItem_Impl*>::iterator it;
...
CGUIGridListItem* CGUIGridList_Impl::GetItem(int iRow, int hColumn)
{
    try
    {
        ...
        // If it exists, get the CGUIGridListItem by using the pool
        if (pItem)
        {
...
{
    CGUIGridListItem* pItem = GetItem(iRow, hColumn);
...
int CGUIGridList_Impl::SetItemText(int iRow, int hColumn, const char* szText, bool bNumber, bool bSection, bool bFast, const char* szSortText)
{
    try
    {
        ...
        // Get the current item at that offset and set the text
        CGUIGridListItem_Impl* pItem = reinterpret_cast<CGUIGridListItem_Impl*>(GetItem(iRow, hColumn));
        if (pItem)
        {
            if (bSection)
            {
...
void CGUIGridList_Impl::SetItemImage(int iRow, int hColumn, CGUIStaticImage* pImage)
{
    ...
    {
        // If it doesn't, create it and set it in the gridlist
        CGUIGridListItem_Impl*  pNewItem = new CGUIGridListItem_Impl(m_pManager, "", CGUIGridListItem_Impl::ImageItem, (CGUIStaticImage_Impl*)pImage);
...
int CGUIGridList_Impl::GetItemColumnIndex(CGUIGridListItem* pItem)
{
    if (pItem)
    {
        try
        {
            CGUIGridListItem_Impl* pItemImpl = (CGUIGridListItem_Impl*)pItem;
...
int CGUIGridList_Impl::GetItemRowIndex(CGUIGridListItem* pItem)
{
    if (pItem)
    {
        try
...
CGUIGridListItem* CGUIGridList_Impl::GetSelectedItem()
{
    return GetListItem(reinterpret_cast<CEGUI::MultiColumnList*>(m_pWindow)->getFirstSelectedItem());
}

CGUIGridListItem* CGUIGridList_Impl::GetNextSelectedItem(CGUIGridListItem* pItem)
{
    if (pItem)
        return GetListItem(reinterpret_cast<CEGUI::MultiColumnList*>(m_pWindow)->getNextSelected(((CGUIGridListItem_Impl*)pItem)->GetListItem()));
    else
        return GetListItem(reinterpret_cast<CEGUI::MultiColumnList*>(m_pWindow)->getFirstSelectedItem());
}
...
int CGUIGridList_Impl::GetSelectedItemRow()
{
    ...
    if (pItem)
    {
        CGUIGridListItem* pListItem = GetListItem(pItem);
        if (pListItem)
        {
...
void CGUIGridList_Impl::SetSelectedItem(int iRow, int hColumn, bool bReset)
{
    ...
    if (pItem != NULL)
    {
        ...
        reinterpret_cast<CEGUI::MultiColumnList*>(m_pWindow)->setItemSelectState(reinterpret_cast<CGUIGridListItem_Impl*>(pItem)->GetListItem(), true);
...
CGUIGridListItem_Impl* CGUIGridList_Impl::GetListItem(CEGUI::ListboxItem* pItem)
{
    CFastHashMap<CEGUI::ListboxItem*, CGUIGridListItem_Impl*>::iterator it;
